...
func (s *Service) NextProblem(ctx context.Context, userID string, sutraID, difficulty int, lessonID string) (*domain.Problem, error) {
	...
	for attempt := 0; attempt < maxDedupAttempts; attempt++ {
		...
		if sutraID == 1 {
			...
			candidate = genSutra2WithLesson(difficulty, lessonID)
		} else if sutraID == 3 {
	...
	if !found {
		...
		if sutraID == 1 {
			...
			p = genSutra2WithLesson(difficulty, lessonID)
		} else if sutraID == 3 {
	...
	for i := 0; i < count; i++ {
		...
		for attempt := 0; attempt < maxDedupAttempts; attempt++ {
			...
			if sutraID == 1 {
				...
				candidate = genSutra2WithLesson(difficulty, "")
			} else if sutraID == 3 {
		...
		if !found {
			if sutraID == 1 {
				...
				p = genSutra2WithLesson(difficulty, "")
			} else if sutraID == 3 {
